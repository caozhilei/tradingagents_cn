...
- 🔬 **研究工具**: 合规的股票研究与策略实验环境
- 🌏 **中文优化**: 完整的中文本地化支持，适配 A 股/港股/美股市场
...
- **数据库**: MongoDB + Redis 双数据库架构，性能提升 10 倍
  - ✅ MongoDB 自动初始化：创建所有集合和索引
  - ✅ Redis 持久化配置：启用 AOF 持久化，数据不丢失
  - ✅ 健康检查机制：确保数据库服务可用
- **部署**: Docker 多架构支持（amd64 + arm64），一键部署
  - ✅ 单命令启动所有服务
  - ✅ 自动处理服务依赖关系
  - ✅ 可选管理界面（MongoDB Express + Redis Commander）

#### 智能体与工作流增强

- 🤖 **智能体可视化配置**: 提供直观的智能体配置界面，支持提示词一键导入和优化
- 🔧 **工具和智能体可视化配置**: 实现工具和智能体之间的可视化配置加载
- 📊 **动态图工作流设计**: 基于动态图的工作流图形化设计功能，拖拽式操作
- ✅ **流程验证和任务发布**: 基于动态流程图架构的流程验证和任务发布机制
...
- **市场分析师**: 技术指标分析（MA、MACD、RSI、BOLL 等）
...
# 启动服务（自动包含前端、后端、MongoDB、Redis）
docker-compose up -d

# 访问应用
# 前端: http://localhost:3000
# 后端API: http://localhost:8000
# MongoDB管理界面: http://localhost:8082 (可选，需添加 -f docker-compose.yml -f docker-compose.hub.nginx.yml)
# Redis管理界面: http://localhost:8081 (可选，需添加 -f docker-compose.yml -f docker-compose.hub.nginx.yml)
```
...
- ✅ **一键部署**: 单命令启动所有服务（前端+后端+MongoDB+Redis）
- ✅ **自动初始化**: MongoDB 自动执行初始化脚本，创建所有集合和索引
- ✅ **数据持久化**: 自动挂载数据卷，确保数据不丢失
- ✅ **健康检查**: 内置健康检查机制，确保服务正常运行
- ✅ **依赖管理**: 自动处理服务间依赖关系，确保按正确顺序启动
- ✅ **可扩展配置**: 支持多种部署模式，满足不同需求

#### 端口映射说明

| 服务            | 容器端口 | 宿主机端口 | 说明                     |
| --------------- | -------- | ---------- | ------------------------ |
| 前端服务        | 80       | 3000       | 主应用访问端口           |
| 后端服务        | 8000     | 8000       | API 服务端口             |
| MongoDB         | 27017    | 27017      | 数据库服务端口           |
| Redis           | 6379     | 6379       | 缓存服务端口             |
| Mongo Express   | 8081     | 8082       | MongoDB 管理界面（可选） |
...
| 部署方式          | 适用场景               | 难度        | 文档链接                                     |
| ----------------- | ---------------------- | ----------- | -------------------------------------------- |
| 🐳 **Docker 版**  | 生产环境、跨平台       | ⭐⭐ 中等   | [Docker 部署指南](./docs/deployment/)        |
...
- 🗄️ [MongoDB 配置说明](./docs/configuration/MongoDB配置说明.md)
  - ✅ 自动初始化：Docker 部署时自动创建所有集合和索引
  - ✅ 数据持久化：自动挂载数据卷
  - ✅ 健康检查：内置健康检查机制
- 📊 [数据源配置指南](./docs/guides/tdx_datasource_configuration.md)
- 🤖 [LLM 提供商配置](./docs/llm/)
- 🐳 [Docker 部署配置](./docs/deployment/)
  - ✅ 一键部署：单命令启动所有服务
  - ✅ 服务依赖：自动处理服务间关系
  - ✅ 可选管理界面：MongoDB Express + Redis Commander
...
### v1.1.1-preview (2026-01-03) - 当前版本 🚀

**重大更新**:

- 🏗️ 架构升级：FastAPI + Vue 3 现代化技术栈
- 🤖 **智能体可视化配置**: 提供直观的智能体配置界面，支持提示词一键导入和优化
- 🔧 **工具和智能体可视化配置**: 实现工具和智能体之间的可视化配置加载
- 📊 **动态图工作流设计**: 基于动态图的工作流图形化设计功能，拖拽式操作
- ✅ **流程验证和任务发布**: 基于动态流程图架构的流程验证和任务发布机制
- 🐳 Docker 增强：集成 MongoDB 和 Redis 服务
- 📋 MongoDB 自动初始化：创建所有集合和索引
- 🔒 Redis 安全配置：启用密码认证和 AOF 持久化

### v1.0.0-preview (2025-12-30)
...
- 🎯 **愿景领导者**: AI 金融领域的前瞻性思考和创新实践
- 💎 **珍贵源码**: 凝聚智慧的开源代码
- 🏗️ **架构大师**: 优雅、可扩展的多智能体框架
- 💡 **技术先驱**: 前沿 AI 技术与金融实务的完美结合
