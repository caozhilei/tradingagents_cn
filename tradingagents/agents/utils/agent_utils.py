    ...
    def get_stock_market_data_unified(
        ...
        try:
            ...
            if is_china:
                ...
                # 数字货币：使用专门的数字货币数据源
                logger.info(f"₿ [统一市场工具] 处理数字货币市场数据...")

                try:
                    # 使用数字货币数据源获取数据
                    from tradingagents.dataflows.interface import get_crypto_data_unified_simple
                    crypto_data = get_crypto_data_unified_simple(ticker, start_date, end_date)

                    # 🔍 调试：打印返回数据的前500字符
                    logger.info(f"🔍 [市场工具调试] 数字货币数据返回长度: {len(crypto_data)}")
                    logger.info(f"🔍 [市场工具调试] 数字货币数据前500字符:\n{crypto_data[:500]}")

                    result_data.append(f"## 数字货币市场数据\n{crypto_data}")
                except Exception as e:
                    logger.error(f"❌ [市场工具调试] 数字货币数据获取失败: {e}")
                    result_data.append(f"## 数字货币市场数据\n获取失败: {e}")
